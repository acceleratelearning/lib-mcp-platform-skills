...
Deploy infrastructure changes with a repeatable safety flow.

1. Confirm stack, AWS account, and region.
2. Install dependencies with pnpm and run Nx pre-deploy checks.
3. Run `pulumi preview` and summarize risk.
4. Run `pulumi up` for the selected stack.
5. Verify outputs and document rollback commands.
```
...
```bash
skills-ref validate ./skills/deploy-pulumi-stack
```
